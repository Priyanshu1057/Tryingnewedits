...
import random
import string
from info import ULTRA_FAST_MODE, MAX_LIST_ELM, BAD_WORDS, LONG_IMDB_DESCRIPTION, IS_VERIFY, MAX_B_TN, TUTORIAL, TUTORIAL_2, TUTORIAL_3, LOG_CHANNEL, TMDB_ON_SEARCH, ADS_MODE
...
async def get_ads_footer():
    """Returns the sponsored-ad footer line for file captions, or '' if no active ad."""
    if not ADS_MODE:
        return ""
    try:
        from database.config_db import mdb
        ads_msg, ads_name, _ = await mdb.get_advirtisment()
        if ads_msg and ads_name:
            ads_url = f"https://telegram.me/{temp.U_NAME}?start=ads"
            return f"\n━━━━━━━━━━━━━━━━━━\n<b><a href='{ads_url}'>{ads_name}</a></b>\n━━━━━━━━━━━━━━━━━━"
    except Exception as e:
        logger.error(f"Error in get_ads_footer: {e}")
    return ""
