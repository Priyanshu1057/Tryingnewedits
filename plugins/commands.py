...
from info import (
    ...
    IS_FILE_LIMIT, FILES_LIMIT, ADS_MODE,
    ...
    try:
        ...
        if len(message.command) == 2 and message.command[1] == "ads":
            ...
            reply_markup = InlineKeyboardMarkup(
                [[InlineKeyboardButton("❌ ᴄꜱᴏꜱᴇ ❌", callback_data="close_data")]]
            )
            if ads_msg:
                await message.reply_photo(
        ...
        # Free file limit check (premium users and allfiles requests are exempt)
        is_allfiles_request = bool(data) and data.startswith("allfiles")
        if IS_FILE_LIMIT and FILES_LIMIT > 0 and not is_allfiles_request and not await db.has_premium_access(user_id):
            ...
            if current_file_count >= FILES_LIMIT:
                buttons = [[
                    InlineKeyboardButton("💎 ɢᴇᴛ ᴩʀᴇᴍɪᴜᴍ", url=f"https://telegram.me/{temp.U_NAME}?start=premium")
                ]]
                return await message.reply_text(
                    f"<b>⚠️ ʏᴏᴜ ʜᴀᴠᴇ ʀᴇᴀᴄʜᴇᴅ ʏᴏᴜʀ ꜰʀᴇᴇ ꜰɪʟᴇ ʟɪᴍɪᴛ!\n\n"
                    f"📊 ᴜꜱᴇᴅ: {current_file_count}/{FILES_LIMIT} ꜰʀᴇᴇ ꜰɪʟᴇꜱ\n\n"
                    f"💎 ᴜᴘɢʀᴀᴅᴇ ᴛᴏ ᴘʀᴇᴍɪᴜᴍ ꜰᴏʀ ᴜɴʟɪᴍɪᴛᴇᴅ ꜰɪʟᴇꜱ!\n"
                    f"ᴏʀ ᴄᴏɴᴛᴀᴄᴛ ᴀᴅᴍɪɴ ᴛᴏ ʀᴇꜱᴇᴛ ʏᴏᴜʀ ʟɪᴍɪᴛ.</b>",
                    reply_markup=InlineKeyboardMarkup(buttons),
                    parse_mode=enums.ParseMode.HTML
                )
...
async def reset_all_limits(client, message):
    try:
        ...
        await message.reply_text(f"<b>❌ ᴇʀʀᴏʀ ʀᴇsᴇᴛᴛɪɴɢ ʟɪᴍɪᴛs: {e}</b>", parse_mode=enums.ParseMode.HTML)
...
async def reset_user_limit(client, message):
    try:
        if len(message.command) < 2:
            return await message.reply_text(
                "<b>❌ ᴜsᴀɢᴇ: /resetuser user_id</b>", parse_mode=enums.ParseMode.HTML
            )
        user_id = int(message.command[1])
        old_limit = await db.get_file_limit(user_id)
        await db.reset_file_limit(user_id)
        await message.reply_text(
            f"<b>✅ ꜰɪʟᴇ ʟɪᴍɪᴛ ʀᴇsᴇᴛ ꜰᴏʀ {user_id}!\n\n"
            f"ᴘʀᴇᴠɪᴏᴜs: {old_limit}\nᴄᴜʀʀᴇɴᴛ: 0</b>",
            parse_mode=enums.ParseMode.HTML
        )
    except ValueError:
        await message.reply_text("<b>❌ ᴘʟᴇᴀsᴇ ɢɪᴠᴇ ᴀ ᴠᴀʟɪᴅ ᴜsᴇʀ ɪᴅ!</b>", parse_mode=enums.ParseMode.HTML)
    except Exception as e:
        await message.reply_text(f"<b>❌ ᴇʀʀᴏʀ: {e}</b>", parse_mode=enums.ParseMode.HTML)
...
async def check_user_limit(client, message):
    try:
        if len(message.command) < 2:
            return await message.reply_text(
                "<b>❌ ᴜsᴀɢᴇ: /checklimit user_id</b>", parse_mode=enums.ParseMode.HTML
            )
        user_id = int(message.command[1])
        current_limit = await db.get_file_limit(user_id)
        await message.reply_text(
            f"<b>📊 ꜰɪʟᴇ ʟɪᴍɪᴛ sᴛᴀᴛᴜs ꜰᴏʀ {user_id}:\n\n"
            f"ᴜsᴇᴅ: {current_limit}/{FILES_LIMIT}\n"
