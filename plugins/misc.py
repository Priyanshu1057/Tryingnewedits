...
async def imdb_search(client, message):
    if " " in message.text:
        ...
        try:
            ...
            logger.error("imdb_search failed: %s", e)
            return await k.edit("IMDb lookup failed, try again later.")
        if not movies:
...
async def imdb_callback(bot: Client, quer_y: CallbackQuery):
    _, movie = quer_y.data.split("#")
    imdb = await get_poster(query=movie, id=True)
    if not imdb:
        return await quer_y.answer("No results", show_alert=True)
...
📆 Yᴇᴀʀ: <a href="{imdb['url']}/releaseinfo">{imdb.get('year', 'N/A')}</a>
🌟 Rᴀᴛɪɴɢ: <a href="{imdb['url']}/ratings">{imdb.get('rating', 'N/A')}</a>/10
"""
    if imdb.get("poster"):
        try:
            await quer_y.message.reply_photo(
                photo=imdb["poster"], caption=caption, reply_markup=InlineKeyboardMarkup(btn)
            )
